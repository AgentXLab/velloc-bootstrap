# Browser Lab Bootstrap

This repository bootstraps the Browser Lab Chromium workspace. It owns the
...
```bash
git clone git@github.com:browser-lab/browser-lab-bootstrap.git
```
...
```bash
git@github.com:browser-lab/browser-lab-chromium.git
```
...
```bash
git@github.com:browser-lab/browser-lab-core.git
```
...
3) **Bootstrap & Sync (clone src + custom_browser + gclient sync)**
   Clones `src/` (Chromium) and `src/custom_browser` (browser-lab-core) if missing,
...
- `CUSTOM_BROWSER_URL` to override the browser-lab-core URL
...
```bash
python scripts/bootstrap_cli.py bootstrap --custom-browser-url git@github.com:browser-lab/browser-lab-core.git
